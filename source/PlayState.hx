	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'purple', 'red', 'ghost', 'sus', 'serenity', 'greedy', 'cheese', 'difficulty', 'khaki']);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss' | 'purple' | 'red' | 'cheese':
						if(isStoryMode && songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
					...
					case 'khaki':
						if(Paths.formatToSongPath(SONG.song) == 'khakimarker' && !usedPractice) {
							unlock = true;

						}
						case 'ghost':
						if(Paths.formatToSongPath(SONG.song) == 'release' && !usedPractice) {
							unlock = true;

						}
						case 'sus':
						if(Paths.formatToSongPath(SONG.song) == 'reactor' && !usedPractice) {
							unlock = true;

						}
						case 'serenity':
						if(Paths.formatToSongPath(SONG.song) == 'inner peace' && !usedPractice) {
							unlock = true;

						}
						case 'greedy':
						if(Paths.formatToSongPath(SONG.song) == 'greediness' && !usedPractice) {
							unlock = true;

						}
						case 'difficulty':
						if(Paths.formatToSongPath(SONG.song) == 'markerous' && !usedPractice) {
							unlock = true;

						}
						case 'cheese':
						if(Paths.formatToSongPath(SONG.song) == 'cheesing' && !usedPractice) {
							unlock = true;

						}
						case 'red':
						if(Paths.formatToSongPath(SONG.song) == 'stained' && !usedPractice) {
							unlock = true;

						}
						case 'purple':
						if(Paths.formatToSongPath(SONG.song) == 'scribbles' && !usedPractice) {
							unlock = true;
